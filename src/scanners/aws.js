...
const { allRules } = require('../rules/aws.js');
  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (user.user === '<root_account>') {
        ...
        if (user.password_last_used !== 'no_information' && user.password_last_used !== 'N/A') {
      ...
      for (const keyNum of ['1', '2']) {
        if (user[`access_key_${keyNum}_active`] === 'true') {
          const lastRotated = new Date(user[`access_key_${keyNum}_last_rotated`]);
 ...
 */
async function scanPolicy(client, policy) {
