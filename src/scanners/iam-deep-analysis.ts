...
/** All techniques by provider */
export interface TechniquesMap {
  aws: PrivescTechnique[];
  azure: PrivescTechnique[];
  gcp: PrivescTechnique[];
}

/**
 * Additional AWS privilege escalation techniques (Rhino Security expanded)
 */
export const ADDITIONAL_AWS_TECHNIQUES: PrivescTechnique[] = [
  // Lambda Layers abuse
  {
 ...
 */
export const ADDITIONAL_AZURE_TECHNIQUES: PrivescTechnique[] = [
  {
    id: 'AzureAD_AddOwner',
    name: 'Add App Owner',
 ...
 */
export const ADDITIONAL_GCP_TECHNIQUES: PrivescTechnique[] = [
  {
 ...
 */
export const DANGEROUS_IAM_PATTERNS: Record<string, DangerousPattern[]> = {
  aws: [
    // Full admin
 ...
 * @param policy - IAM policy document
    ...
    for (const action of actions) {
      ...
      for (const pattern of patterns) {
        if (matchPattern(action, pattern.pattern)) {
          ...
          const severity: Severity | 'info' = hasWildcardResource
            ? pattern.severity
 ...
 * @param findings - Policy analysis findings
 * @returns Risk score and breakdown
 */
export function calculateRiskScore(findings: PolicyFinding[]): RiskScoreResult {
  let score = 0;
  ...
  for (const finding of findings) {
    ...
    breakdown.categories[finding.category] = (breakdown.categories[finding.category] || 0) + 1;
  }
 ...
 */
export function getAllTechniques(): TechniquesMap {
  return {
