...
export interface IAMData {
  users?: Array<{ arn: string; name: string }>;
  roles?: Array<{ arn: string; name: string; trustPolicy?: TrustPolicy }>;
...
export const AWS_PRIVESC_TECHNIQUES: PrivescTechnique[] = [
  ...
  },
  {
    ...
    description:
      'Attacker can modify trust policy to allow themselves to assume high-privilege roles',
    severity: 'critical',
  ...
  // Role Assumption Chains
  {
    ...
    description:
      'Attacker can launch EC2 with high-privilege role and access from instance metadata',
    severity: 'critical',
 ...
 */
export function detectPrivescPaths(
  provider: string,
  permissions: string[],
  ...
  let techniques: PrivescTechnique[];
 ...
 */
export function buildAttackGraph(iamData: IAMData): AttackGraph {
 ...
 */
function extractPrincipals(principal: unknown): string[] {
  if (!principal) return [];
  ...
  if (p.AWS) {
    ...
    principals.push(...(aws as string[]));
  }
  if (p.Service) {
    const svc = Array.isArray(p.Service) ? p.Service : [p.Service];
    principals.push(...(svc as string[]).map(s => `Service:${s}`));
  }
  if (p.Federated) {
    const fed = Array.isArray(p.Federated) ? p.Federated : [p.Federated];
    principals.push(...(fed as string[]).map(f => `Federated:${f}`));
  }
