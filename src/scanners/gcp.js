...
async function scanGCP(options = {}) {
  ...
  try {
    ...
    if (error.code === 'MODULE_NOT_FOUND') {
      console.error('GCP SDK not installed. Run: npm install googleapis @google-cloud/iam-credentials @google-cloud/resource-manager');
 ...
 */
async function scanProjectIAMPolicy(cloudresourcemanager, projectId) {
  const findings = [];
    ...
    for (const binding of policy.bindings || []) {
      ...
      if (members.includes('allAuthenticatedUsers')) {
        findings.push({
          ...
          recommendation: 'This grants access to any Google account - restrict to specific principals',
        });
      }
      ...
      if (delegationRoles.includes(role)) {
        for (const member of members) {
      ...
      const dangerousRoles = [
        ...
        { role: 'roles/editor', severity: 'warning', msg: 'Project editor access (can modify most resources)' },
        { role: 'roles/iam.securityAdmin', severity: 'warning', msg: 'Can manage all IAM policies' },
        { role: 'roles/resourcemanager.projectIamAdmin', severity: 'warning', msg: 'Can manage project IAM' },
 ...
 */
async function scanServiceAccounts(iam, projectId) {
  const findings = [];
  ...
  try {
    ...
    for (const sa of serviceAccounts) {
      ...
      if (sa.email?.includes('gserviceaccount.com') && 
      ...
      try {
        ...
        for (const binding of saPolicy.data.bindings || []) {
          const role = binding.role;
          const members = binding.members || [];
 ...
 */
async function scanCustomRoles(iam, projectId) {
  const findings = [];
  ...
  try {
    ...
    const dangerousPermissions = [
      { perm: 'iam.serviceAccountKeys.create', severity: 'critical', msg: 'Can create service account keys' },
      { perm: 'iam.serviceAccountTokenCreator', severity: 'critical', msg: 'Can create tokens for service accounts' },
      { perm: 'iam.serviceAccounts.actAs', severity: 'warning', msg: 'Can act as service accounts' },
      { perm: 'iam.serviceAccounts.getAccessToken', severity: 'critical', msg: 'Can get access tokens' },
      { perm: 'iam.serviceAccounts.signBlob', severity: 'warning', msg: 'Can sign blobs as service accounts' },
      { perm: 'iam.serviceAccounts.signJwt', severity: 'warning', msg: 'Can sign JWTs as service accounts' },
      { perm: 'resourcemanager.projects.setIamPolicy', severity: 'critical', msg: 'Can modify project IAM policies' },
      { perm: 'deploymentmanager.deployments.create', severity: 'warning', msg: 'Can create deployments (potential code execution)' },
      { perm: 'cloudfunctions.functions.create', severity: 'warning', msg: 'Can create Cloud Functions (code execution)' },
      { perm: 'cloudfunctions.functions.setIamPolicy', severity: 'warning', msg: 'Can set Cloud Functions IAM' },
      { perm: 'compute.instances.setServiceAccount', severity: 'warning', msg: 'Can change VM service accounts' },
      { perm: 'run.services.setIamPolicy', severity: 'warning', msg: 'Can set Cloud Run IAM' },
      { perm: 'storage.buckets.setIamPolicy', severity: 'warning', msg: 'Can set bucket IAM policies' },
    ];
    ...
    for (const role of roles) {
      ...
      const canCreateKeys = permissions.includes('iam.serviceAccountKeys.create');
