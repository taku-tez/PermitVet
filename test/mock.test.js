...
describe('AWS Scanner with Mocks', () => {
  ...
  it('detects user without MFA', async () => {
    ...
    const mockIAM = {
      listUsers: async () => ({
        Users: [
          { UserName: 'admin', UserId: 'AIDA123', CreateDate: new Date() }
        ]
      }),
  ...
  it('detects old access keys', async () => {
    ...
    const daysSince = Math.floor((Date.now() - accessKey.CreateDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  it('detects wildcard actions in policy', () => {
    ...
    const hasWildcardAction = policy.Statement.some(s => 
...
describe('GCP Scanner with Mocks', () => {
  ...
  it('detects dangerous custom role permissions', () => {
    ...
    const hasDangerous = role.includedPermissions.some(p => 
...
describe('Privilege Escalation Detection', () => {
  const { detectPrivescPaths, AWS_PRIVESC_TECHNIQUES } = require('../src/scanners/privesc-detector.js');
  ...
  it('detects CreatePolicyVersion privesc', () => {
    ...
    const hasCreatePolicyVersion = paths.some(p => 
  ...
  it('detects PassRole + Lambda privesc combo', () => {
    ...
    const hasLambdaPrivesc = paths.some(p => 
...
describe('Compliance Mapping', () => {
  const { mapToCompliance, generateComplianceSummary } = require('../src/compliance.js');
...
describe('Reporter Output', () => {
  const { generateSARIF, generateHTMLReport } = require('../src/compliance.js');
